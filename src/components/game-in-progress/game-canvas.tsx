...
export class GameCanvas extends React.Component<any, Sprops> {
    ...
    countDown = () => {
        ...
        if (c_seconds) {
            ...
            this.setState({outcome: this.checkGameOver() ? Outcome.WIN : Outcome.LOST,});
    ...
    checkSelection(x: number, y: number): boolean {
        return (
            (!!this.state.start && this.state.start.x === x && this.state.start.y === y) ||
    ...
    handleSelectedTiles() {
        ...
        if (dfs(grid, start?.x as number, start?.y as number, grid.length, grid[0].length,
            ...
            fetchAllPaths(grid, start?.x as number, start?.y as number, grid.length, grid[0].length, 0, "", end?.x as number, end?.y as number, visitedForAllPaths, stackForAllPaths, shortestStack);
    ...
    drawLines(path: number[][]) {
        ...
        for (let i = 0; i < myDivs.length - 1; i++) {
            ...
            var cy = (q + s) / 2 - 4 / 2;
            var htmlLine = "<div style='padding:0px; margin:0px; height:2px; background-color:red ; line-height:1px; position:absolute; left:" + cx + "px; top:" + cy + "px; width:" + length +
                "px; -moz-transform:rotate(" + angle + "deg); -webkit-transform:rotate(" + angle + "deg); -o-transform:rotate(" + angle + "deg); -ms-transform:rotate(" + angle + "deg); transform:rotate(" + angle + "deg);' />";
            // @ts-ignore
                                ...
                                {row.map((item, y) =>
                                    item !== 0 ? (
                                        <div className={`cell ${this.checkSelection(x, y) ? "selected" : null}`}
                                            id={`${x}${y}`} onClick={() => this.handleClick(x, y, item)}>{item}</div>
                                    ) : (<div className="hidden" id={`${x}${y}`}>{item}</div>)
                                )}
                            </div>
                        ))}
                    </div>
